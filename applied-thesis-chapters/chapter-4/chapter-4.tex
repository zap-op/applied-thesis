...
\begin{itemize}
  ...
  Để đảm bảo tài nguyên của phiên quét loại ZAP Ajax được kiểm soát tốt, nhóm đặt các cấu hình mặc định, không bao gồm trong tham số config, trên ZAP instance trước khi start phiên quét. Các cấu hình như sau:
        ...
        \begin{tabularx}{\textwidth}{|>{\hsize=.15\hsize\centering\let\newline
          ...
          Trong phạm vi giải pháp, vì chưa cung cấp giải pháp cấu hình phạm vi nên giá trị của tham số này chưa thể thay đổi khác "false".
          \\
          \hline
          contextName
           &
...
Để biết được target nào cần quét, sử dụng các loại quét nào và có cấu hình tùy chỉnh như thế nào.
Trong store, nhóm tạo đối tượng listSelectedTarget để lưu danh sách các target được chọn, đối tượng scanOption để lưu thông tin trạng thái các lựa chọn quét và đối tượng scanConfig để lưu thông tin trạng thái các cấu hình quét tùy chỉnh.
Đối tượng listSelectedTarget là một mảng có các phần tử là đối tượng chứa một số thông tin của target, thông tin tiêu biểu là target id.
Đối tượng scanOption là một đối tượng chứa thông tin trạng thái bật tắt của các loại quét.
Đối tượng scanConfig cũng tương tự như đối tượng scanOption nhưng dùng để chứa cấu hình.
Cùng với đó, nhóm tạo các reducer tương ứng trong store để thực hiện thay đổi, thêm bớt cho các đối tượng này.

Ở giao diện web, theo cách tạo phiên quét bình thường, người dùng phải trải qua 3 bước thiết lập là chọn các target trong danh sách, chọn loại quét và có thể tùy chọn điều chỉnh cấu hình của phiên quét.
...
Hook gọi đến lần lượt các trạng thái trong scanOption, ở mỗi trạng thái nếu là true thì sẽ duyệt listSelectedTarget, tạo phiên quét tương ứng với id target và cấu hình đó, gọi đến endpoint dùng để quét đã trình bày ở trên.
