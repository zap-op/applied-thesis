...
\begin{itemize}
    ...
    \item Tham chiếu đối tượng không an toàn (Insecure Direct Object Reference - IDOR). Đối tượng có thể được tham chiếu qua nhiều phương pháp, tuy nhiên định danh duy nhất của đối tượng dùng để tham chiếu lại quá dễ đoán, không được tiền xử lý như hình 
    \begin{figure}[H]
        \centering
...
\begin{itemize}
    ...
    \item Liệu có bất cứ một thuật toán hoặc giao thức mật mã cũ hoặc yếu nào được sử dụng trong cài đặt mặc định hoặc trong code cũ không?
...
\begin{itemize}
    ...
    \item Các cài đặt bảo mật trong các máy chủ ứng dụng, các framework ứng dụng (ví dụ: Struts, Spring, ), thư viện, cơ sở dữ liệu, v.v. không được cài đặt với các thiết lập an toàn.
...
\section{Kiến trúc phân tầng (N-Tier)}
...
\begin{itemize}
    ...
    \item Ứng dụng có thể hoạt động trên nhiều môi trường, hệ điều hành, tech stack khác nhau.
\end{itemize}
...
\begin{itemize}
    ...
    \item Hệ thống IaaS khó quản lý và cần nhiều thời gian tìm hiểu, học hỏi.
...
\begin{itemize}
    \item Để người dùng có thể theo dõi được trạng thái của của phiên quét và để ứng dụng phía người dùng thể thực hiện các tác . Nhóm chọn thiết kế giải nguồn sự kiện (Event Sourcing).
    \begin{figure}[H]
        \centering
    ...
    \item The Event Sourcing pattern involves sending a continuous stream of messages to an authoritative storage target. Each message describes an event in the system. Services and applications then query the event store for interesting events in a way that is particular to the purpose of the given service or application.
    ...
    \item Event source nhóm sử dụng ở đây là SSE (Server Sent Event) được dựa trên phương thức giao tiếp HTTP.
\end{itemize}
